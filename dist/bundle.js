(function (global, factory) {
   ...
   * @return {Promise}
   */
  async function saveTile(tileInfo, blob, timestamp, group) {
    return (await dbPromise).put(tileStoreName, Object.assign({}, {timestamp: timestamp,
      group: group,
      blob: blob},
      tileInfo));
  }
  ...
  function getTileUrls(layer, bounds, zoom) {
    ...
    for (var j = tileBounds.min.y; j <= tileBounds.max.y; j += 1) {
      for (var i = tileBounds.min.x; i <= tileBounds.max.x; i += 1) {
        ...
        var data = Object.assign({}, layer.options,
          {x: i,
          y: j,
          z: zoom});
        tiles.push({
